...
def doTest(remote, application, datastore)
  ...
  test_id = `date +%Y%m%d%H%M`.chomp + "_" + application + "_" + datastore 
  1.times { |i| 
    # Start Agents  
  ...
  remote.processLogs(test_id, datastore, 3, 1)
...
#application, datastore = "appbenchguestbook", "bigtable"
application, datastore = "appbenchpetlog", "bigtable"
